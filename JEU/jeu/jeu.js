const ASSETS = {
      ...
      function update(step) {
        ...
        if (KEYS.ArrowRight)
        deplacerVoitureDroite()
        else if (KEYS.ArrowLeft)
        deplacerVoitureGauche()
        ...
        const socket = new WebSocket('ws://localhost:8080/game');
              ...
              function deplacerVoitureGauche() {
                direction = "gauche"
                  sendMessage('Gauche');

              }
        
              function deplacerVoitureDroite() {
                direction = "droite"
                  sendMessage('Droite');
              }
              ...
              socket.onmessage = function(event) {
                ...
                if (messageParts.length === 2 && messageParts[0] === 'PositionVoiture' && direction === "droite") {
                    const newPosition = parseInt(messageParts[1]);
                    (hero.style.backgroundPosition = "-220px 0"),
                    (playerX += newPosition/1000000 * step * speed);
                    updateMainCarPosition(newPosition);
                }
                else if (messageParts.length === 2 && messageParts[0] === 'PositionVoiture' && direction === "droite") {
                  const newPosition = parseInt(messageParts[1]);
                ...
                function updateMainCarPosition(newPosition) {
                    const mainCarElement = document.getElementById('mainCar');
                    if (mainCarElement) {
                        mainCarElement.style.left = newPosition + 'px';
                    }
                }
              ...
              document.addEventListener('keydown', function(event) {
                if (event.code === 'ArrowRight') {
                    // Appeler la fonction de déplacement vers la droite
                    deplacerVoitureDroite();
                }
            });
